...
원본은 [qwibitai/nanoclaw](https://github.com/qwibitai/nanoclaw)에서 출발했지만, 현재는 RBClaw의 Discord/paired-runtime 구조에 맞게 독립적으로 유지되고 있습니다.
...
- owner / reviewer / arbiter는 채널에 지정된 하나의 `workDir`을 공유합니다.
- reviewer / arbiter는 같은 폴더를 mount namespace 안에서 read-only로 읽습니다.
...
- reviewer와 arbiter는 호스트 홈과 `workDir`를 읽기 전용으로 잠그고 역할 세션·IPC 경로만 쓰기 허용한 mount namespace에서 실행됩니다.
...
- Linux(Ubuntu 22.04+)
- reviewer / arbiter 격리를 위한 user namespace와 `/usr/bin/unshare`
...
4. `systemctl --user restart rbclaw`
