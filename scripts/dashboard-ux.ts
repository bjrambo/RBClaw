...
async function runDesktopDashboardScenarios(browser: Browser, baseUrl: string) {
  ...
  await runScenario(
    ...
    async (page, state) => {
      ...
      const canScroll = await detail.evaluate(
...
async function runScenarioWithViewport(
  ...
  await context.addInitScript(
    "localStorage.setItem('rbclaw.dashboard.locale.v2', 'ko')",
  );
...
async function handleMockApi(route: Route, state: MockApiState) {
  ...
  if (method === 'GET' && url.pathname === '/api/rooms/dc%3Aops/timeline') {
    await fulfillJson(route, mockRoomActivity(state));
    return;
  }
...
function mockRoomActivity(state: MockApiState) {
  return {
...
function runtimeInventoryFixture() {
  ...
  return {
    ...
    codex: {
      ...
      mcp: { configPath: codexConfig, rbclawConfigured: true, serverCount: 1 },
    },
    claude: {
      ...
      mcp: {
        ...
        rbclawConfigured: false,
        serverCount: 0,
      },
    },
    rbclaw: {
      runnerSkillDir: {
        label: 'RBClaw runner skills',
