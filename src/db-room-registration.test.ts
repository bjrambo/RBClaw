...
describe('registered group isMain', () => {
  ...
  it('repairs only main rooms that have no configured workDir', () => {
    assignRoom('dc:main-missing-work-dir', {
      name: 'Main Room',
      roomMode: 'single',
    ...
    assignRoom('dc:regular-missing-work-dir', {
      name: 'Regular Room',
      roomMode: 'single',
      ownerAgentType: 'codex',
      folder: 'regular-room',
    });
    assignRoom('dc:main-existing-work-dir', {
      name: 'Configured Main Room',
      roomMode: 'single',
      ownerAgentType: 'codex',
      folder: 'configured-main-room',
      isMain: true,
      workDir: '/srv/configured-main',
    });

    expect(repairMissingMainRoomWorkDirs('/srv/rbclaw')).toBe(1);
    expect(getRegisteredGroup('dc:main-missing-work-dir')?.workDir).toBe(
      '/srv/rbclaw',
    );
    expect(getRegisteredGroup('dc:regular-missing-work-dir')?.workDir).toBe(
      undefined,
    );
    expect(getRegisteredGroup('dc:main-existing-work-dir')?.workDir).toBe(
      '/srv/configured-main',
    );
    expect(repairMissingMainRoomWorkDirs('/srv/other')).toBe(0);
  });
