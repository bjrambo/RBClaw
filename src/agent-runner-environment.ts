...
function getOwnerRunnerSkillSources(projectRoot: string): Array<{
  dir: string;
  scope: 'runner';
}> {
  return [
    ...
    {
      dir: path.join(projectRoot, 'runners', 'local-skills'),
      scope: 'runner',
    },
  ];
}
...
function upsertRbclawMcpServerSection(args: {
  ...
  const stripRbclawMcpServerSections = (input: string): string => {
    ...
    for (const line of lines) {
      ...
      const isRbclawSection = /^\[mcp_servers\.rbclaw(?:\.[^\]]+)?\]$/.test(
        trimmed,
      );
  ...
  let toml = fs.existsSync(args.sessionConfigPath)
    ...
    : '';
  toml = stripRbclawMcpServerSections(toml);
...
${tomlEnvLine(RBCLAW_ENV.ipcDir, args.ipcDir)}
${tomlEnvLine(RBCLAW_ENV.hostIpcDir, args.hostIpcDir)}
${tomlEnvLine(RBCLAW_ENV.chatJid, args.chatJid)}
${tomlEnvLine(RBCLAW_ENV.groupFolder, args.groupFolder)}
${tomlEnvLine(RBCLAW_ENV.isMain, args.isMain ? '1' : '0')}
${tomlEnvLine(RBCLAW_ENV.agentType, args.agentType)}
${args.roomRole ? `${tomlEnvLine(RBCLAW_ENV.roomRole, args.roomRole)}\n` : ''}
${args.workDir ? `${tomlEnvLine(RBCLAW_ENV.workDir, args.workDir)}\n` : ''}
`;
...
function buildBaseRunnerEnv(args: {
  ...
  delete cleanEnv[RBCLAW_ENV.workDir];
  ...
  return {
    ...
    TZ: TIMEZONE,
    HOME: os.homedir(),
    [RBCLAW_ENV.groupDir]: args.groupDir,
    [RBCLAW_ENV.ipcDir]: args.groupIpcDir,
    [RBCLAW_ENV.hostIpcDir]: args.hostIpcDir,
    [RBCLAW_ENV.globalDir]: args.globalDir,
    ...(args.group.workDir ? { [RBCLAW_ENV.workDir]: args.group.workDir } : {}),
    [RBCLAW_ENV.chatJid]: args.chatJid,
    [RBCLAW_ENV.groupFolder]: args.group.folder,
    [RBCLAW_ENV.isMain]: args.isMain ? '1' : '0',
    [RBCLAW_ENV.agentType]: args.agentType,
    CLAUDE_CONFIG_DIR: args.groupSessionsDir,
    ...(args.runtimeTaskId
      ? { [RBCLAW_ENV.runtimeTaskId]: args.runtimeTaskId }
      : {}),
  };
}
  ...
  try {
    ...
    const sessionAgents = (
      args.useFailoverPromptPack
        ? [
            readCustomPrompt(args.projectRoot),
    ...
    if (hasDisabledSkillOverrides(args.skillOverrides, 'codex')) {
      ...
      syncRoomSkillDirectories({
        sources: [
          ...
          ...getOwnerRunnerSkillSources(args.projectRoot),
        ],
      ...
      syncDirectoryEntries(
        [
          ...
          path.join(args.projectRoot, 'runners', 'local-skills'),
        ],
        codexSkillsDir,
      );
    }
    ...
    if (fs.existsSync(mcpServerPath)) {
      upsertRbclawMcpServerSection({
        sessionConfigPath,
        mcpServerPath,
        ipcDir: args.env[RBCLAW_ENV.ipcDir],
        hostIpcDir: args.env[RBCLAW_ENV.hostIpcDir],
        chatJid: args.chatJid,
        groupFolder: args.group.folder,
        isMain: args.isMain,
        agentType: 'codex',
        roomRole: args.roomRole,
        workDir: args.env[RBCLAW_ENV.workDir] || args.group.workDir,
      });
    }
  ...
  syncRoomSkillDirectories({
    sources: [
      ...
      ...(workDirClaude
        ...
        : []),
      ...getOwnerRunnerSkillSources(projectRoot),
    ],
...
export function prepareReadonlySessionEnvironment(args: {
  ...
  const isolatedHomeDir = path.join(sessionDir, 'home');
  ...
  if (agentType === 'codex') {
    syncRoomSkillDirectories({
      ...
      destination: path.join(isolatedHomeDir, '.agents', 'skills'),
      agentType,
      overrides: skillOverrides,
    });
  }
