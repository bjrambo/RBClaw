...
describe('claude-session-settings', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rbclaw-claude-settings-'));
    hostSettingsPath = path.join(tempDir, 'host-settings.json');
    process.env.RBCLAW_CLAUDE_SETTINGS_PATH = hostSettingsPath;
  });

  afterEach(() => {
    delete process.env.RBCLAW_CLAUDE_SETTINGS_PATH;
