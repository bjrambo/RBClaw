...
describe('message-runtime-follow-up', () => {
  ...
  it('does not schedule an owner follow-up after an arbiter escalates to the user', () => {
    const enqueue = vi.fn();
    const result = dispatchPairedFollowUpForEvent({
      chatJid: 'group@test',
      runId: 'run-arbiter-escalate',
      task: {
        id: 'task-arbiter-escalate',
        status: 'active',
        round_trip_count: 1,
        updated_at: '2026-03-30T00:00:00.000Z',
      ...
      fallbackLastTurnOutputRole: 'arbiter',
      enqueue,
    });
  ...
  it('consumes bot-only arbiter escalation follow-ups without waking the owner', () => {
    const enqueue = vi.fn();
    const result = dispatchPairedFollowUpForEvent({
      chatJid: 'group@test',
      runId: 'run-bot-only-arbiter-escalate',
      task: {
        id: 'task-bot-only-arbiter-escalate',
        status: 'active',
        round_trip_count: 1,
        updated_at: '2026-03-30T00:00:00.000Z',
        arbiter_verdict: 'escalate',
