...
describe('formatMessages', () => {
  ...
  it('marks voice companion messages with approval metadata', () => {
    const result = formatMessages(
      [
        makeMsg({
          sender_name: 'Test Voice Companion',
          content: '배포 상태 확인',
          message_source_kind: 'voice_companion',
    ...
    expect(result).toContain('source_kind="voice_companion"');
    expect(result).toContain('approval_level="high_risk_confirmed"');
    expect(result).toContain('approval_method="local_confirm_click"');
  });
});
