...
function normalizeDashboardAgentType(value: unknown): AgentType | null {
  return value === 'claude-code' || value === 'codex' || value === 'glm-code'
    ? value
    : null;
}

export function resolveDashboardEntryAgentType(args: {
  entryAgentType: unknown;
  snapshotAgentType: unknown;
}): AgentType {
  return (
    normalizeDashboardAgentType(args.entryAgentType) ??
    normalizeDashboardAgentType(args.snapshotAgentType) ??
    'claude-code'
  );
}

export function findCodexUsageSnapshot(
  snapshots: StatusSnapshot[],
): StatusSnapshot | undefined {
  return snapshots.find(
    (snapshot) =>
      snapshot.serviceId === 'codex-main' ||
      snapshot.serviceId === 'codex' ||
      snapshot.usageRows?.some((row) => /^Codex\b/.test(row.name)),
  );
}
  ...
  for (const snapshot of snapshots) {
    for (const entry of snapshot.entries) {
      ...
      existing.push({
        ...
        agentType: resolveDashboardEntryAgentType({
          entryAgentType: entry.agentType,
          snapshotAgentType: snapshot.agentType,
        }),
        status: entry.status,
...
async function buildUsageContent(): Promise<string> {
  ...
  if (cachedCodexUsageRows.length > 0) {
    ...
    const codexSnapshot = findCodexUsageSnapshot(
      readStatusSnapshots(STATUS_SNAPSHOT_MAX_AGE_MS),
    );
    codexBotRows.push(
