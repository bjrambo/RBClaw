...
vi.mock('./config.js', () => ({
  DATA_DIR: '/tmp/rbclaw-codex-usage-data',
}));
...
describe('codex-usage-collector fallback account usage', () => {
  ...
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
    tempHome = fs.mkdtempSync(path.join('/tmp', 'rbclaw-codex-usage-'));
  ...
  it('full scan refreshes usage for a single fallback Codex account', async () => {
    createDefaultCodexAuth(tempHome);
    const childProcess = await import('child_process');
    const fallbackCodexHome = path.join(tempHome, '.codex');
    let capturedCodexHome: string | undefined;
  ...
  it('reads live wham usage before falling back to app-server usage', async () => {
    createDefaultCodexAuth(tempHome);
    const childProcess = await import('child_process');
    ...
    vi.mocked(globalThis.fetch).mockResolvedValue({
      ...
      json: vi.fn().mockResolvedValue({
        ...
        rate_limit: {
          ...
          secondary_window: {
            reset_at: Math.floor((Date.now() + 86_400_000) / 1000),
            used_percent: 18,
          },
        },
      }),
