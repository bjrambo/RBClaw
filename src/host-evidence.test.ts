...
describe('host evidence helpers', () => {
  it('recognizes only allowlisted actions', () => {
    expect(isHostEvidenceAction('rbclaw_service_status')).toBe(true);
    expect(isHostEvidenceAction('rbclaw_service_logs')).toBe(true);
    expect(isHostEvidenceAction('rbclaw_role_runtime_config')).toBe(true);
    ...
    expect(isHostEvidenceAction('rbclaw_deploy_state')).toBe(true);
  ...
  it('builds deterministic commands for each allowlisted action', () => {
    ...
    const logs = buildHostEvidenceCommand({
      requestId: 'req-2',
      action: 'rbclaw_service_logs',
      tailLines: 42,
    });
    expect(logs.file).toBe('journalctl');
    expect(logs.args).toEqual(
      expect.arrayContaining(['--user', '-u', 'rbclaw', '-n', '42']),
    );
  });
