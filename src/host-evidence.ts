...
export interface HostEvidenceResult {
  ok: boolean;
  action: HostEvidenceAction | 'rbclaw_room_runtime';
  command: string;
  stdout: string;
  stderr: string;
  exitCode: number;
...
export function buildRoleRuntimeConfigEvidence(): string {
  return JSON.stringify(
    {
      action: 'rbclaw_role_runtime_config',
      current_service: {
...
export async function runHostEvidenceRequest(
  ...
  let commandText = '';
  try {
    if (request.action === 'rbclaw_role_runtime_config') {
    ...
    if (request.action === 'rbclaw_deploy_state') {
      commandText = `internal:${request.action}`;
      return {
        ok: true,
        action: request.action,
    ...
    if (request.action === 'rbclaw_artifact_metadata') {
      commandText = `internal:${request.action}`;
      return {
        ok: true,
        action: request.action,
