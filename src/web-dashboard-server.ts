...
const STACK_RESTART_UNIT_NAME = 'rbclaw-stack-restart.service';
const MANAGED_SERVICE_CALLER_FALLBACK_MESSAGE =
  'Stack restart unit is not installed yet. Run setup service from an external shell before retrying from a managed RBClaw service.';
...
function requiresConfiguredDashboardToken(url: URL, request: Request): boolean {
  return (
...
function rejectUnauthorizedApi(
  ...
  if (!requiresConfiguredDashboardToken(url, request)) return null;
  if (!token) {
    return jsonResponse(
      {
        error:
          'WEB_DASHBOARD_TOKEN is required for mutating dashboard API routes',
      },
      { status: 503 },
    );
  }
...
export function createWebDashboardHandler(
  ...
  return async (request: Request): Promise<Response> => {
    ...
    const availableGroupsRoute = handleAvailableGroupsRoute({
      url,
      request,
      jsonResponse,
      loadRoomBindings,
    ...
    if (availableGroupsRoute) return availableGroupsRoute;
...
export function startWebDashboardServer(
  ...
  const server = Bun.serve({
    ...
    fetch: createWebDashboardHandler({
      ...
      publishRoomMessage: opts.publishRoomMessage,
