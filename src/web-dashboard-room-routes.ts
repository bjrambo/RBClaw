...
function buildFallbackRoomActivity(
  deps: RoomsTimelineRouteDependencies,
  jid: string,
  binding: RegisteredGroup,
): WebDashboardRoomActivity {
  const snapshot: StatusSnapshot = {
    serviceId: 'web-dashboard',
    agentType: binding.agentType ?? 'codex',
    assistantName: 'Dashboard',
    updatedAt: new Date().toISOString(),
    entries: [],
  };
  const entry: StatusSnapshot['entries'][number] = {
    jid,
    name: binding.name,
  ...
  return buildRoomActivity(deps, snapshot, entry);
}
...
function handleSingleRoomTimeline({
  ...
  if (!matched) {
    const binding = deps.loadRoomBindings?.()[deps.timelineRoomJid];
    if (binding) {
      return jsonResponse(
        buildFallbackRoomActivity(deps, deps.timelineRoomJid, binding),
      );
    }
