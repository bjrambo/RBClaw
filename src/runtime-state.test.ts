...
const db = vi.hoisted(() => ({
  ...
  repairMissingMainRoomWorkDirs: vi.fn(() => 0),
...
describe('createRuntimeState', () => {
  ...
  it('repairs a missing main-room workDir before loading room bindings', () => {
    const state = createRuntimeState();

    state.loadState();

    expect(db.repairMissingMainRoomWorkDirs).toHaveBeenCalledWith(
      process.cwd(),
    );
    expect(
      db.repairMissingMainRoomWorkDirs.mock.invocationCallOrder[0],
    ).toBeLessThan(db.getAllRoomBindings.mock.invocationCallOrder[0]!);
  });
});
