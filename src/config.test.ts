...
describe('config/env loading', () => {
  ...
  beforeEach(() => {
    ...
    tempRoot = fs.realpathSync(
      fs.mkdtempSync(path.join('/tmp', 'rbclaw-config-')),
    );
    previousCwd = process.cwd();
    previousEnv = { ...process.env };
    process.chdir(tempRoot);
    delete process.env.RBCLAW_STORE_DIR;
    delete process.env.RBCLAW_GROUPS_DIR;
    delete process.env.RBCLAW_DATA_DIR;
    delete process.env.RBCLAW_CACHE_DIR;
  ...
  it('loads formerly process-only config values through the shared env path', async () => {
    fs.writeFileSync(
      path.join(tempRoot, '.env'),
      [
        'RBCLAW_STORE_DIR=./custom-store',
        'RBCLAW_GROUPS_DIR=./custom-groups',
        'RBCLAW_DATA_DIR=./custom-data',
        'RBCLAW_CACHE_DIR=./custom-cache',
        'AGENT_TIMEOUT=12345',
        'LOG_LEVEL=trace',
