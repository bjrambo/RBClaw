...
async function acquirePreparedWorkDirLock(
  preparedExecution: PreparedMessageAgentExecution,
): Promise<(() => void) | null> {
  const workDir =
    preparedExecution.pairedExecutionContext?.workDir ??
    preparedExecution.effectiveGroup.workDir;
  return workDir ? acquireWorkDirExecutionLock(workDir) : null;
}
...
export async function runAgentForGroup(
  ...
  const releaseWorkDirLock =
    await acquirePreparedWorkDirLock(preparedExecution);
  ...
  const runAttempt = (currentProvider: 'claude' | 'codex') =>
    runMessageAgentAttempt({
      ...
      roomRoleContext,
      pairedExecutionContext,
      fallbackWorkDir: group.workDir ?? null,
