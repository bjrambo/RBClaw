...
function resolveTaskArbiterVisibleVerdict(
  task: Pick<PairedFollowUpTaskContext, 'status' | 'arbiter_verdict'> | null,
): VisibleVerdict | null {
  if (task?.status !== 'active') {
    return null;
  }
  return task.arbiter_verdict === 'escalate' ? 'escalate' : null;
}
...
export function resolvePairedFollowUpDecision(args: {
  ...
  const taskArbiterVerdict = resolveTaskArbiterVisibleVerdict(
    args.task ?? null,
  );
  const fallbackLastTurnOutputRole =
    args.fallbackLastTurnOutputRole ?? (taskArbiterVerdict ? 'arbiter' : null);
  const fallbackLastTurnOutputVerdict =
    args.fallbackLastTurnOutputVerdict ?? taskArbiterVerdict;
  const { role: lastTurnOutputRole, verdict: lastTurnOutputVerdict } =
    resolveLatestPairedTurnOutputContext({
      task: args.task,
      fallbackLastTurnOutputRole,
