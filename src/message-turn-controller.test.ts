...
describe('MessageTurnController progress edit guard and failure finals', () => {
  ...
  it('publishes a failure final for a silent single-room error', async () => {
    const channel = {
      ...makeChannel(),
      name: 'discord',
    } satisfies Channel;
    const deliverFinalText = vi.fn().mockResolvedValue(true);
    const controller = new MessageTurnController({
      chatJid: 'dc:single-room',
      group: makeGroup(),
      runId: 'run-single-room-silent-error',
      channel,
      idleTimeout: 1_000,
      failureFinalText: '실패',
      ...
      requestClose: vi.fn(),
      deliverFinalText,
      publishFailureFinalWhenSilent: true,
    });
  ...
  it('keeps paired silent failures invisible for the existing retry path', async () => {
    const channel = {
      ...makeChannel(),
      name: 'discord',
    } satisfies Channel;
    const deliverFinalText = vi.fn().mockResolvedValue(true);
    const controller = new MessageTurnController({
      chatJid: 'dc:paired-room',
      group: makeGroup(),
      runId: 'run-paired-room-silent-error',
      channel,
      idleTimeout: 1_000,
      failureFinalText: '실패',
      isClaudeCodeAgent: false,
      ...
      requestClose: vi.fn(),
      deliverFinalText,
      publishFailureFinalWhenSilent: false,
    });
