...
describe('runAgentForGroup paired turn revision guards', () => {
  ...
  it('allows a claimed paired turn revision when preparation advances the task before execution', async () => {
    ...
    expect(envOverrides).toEqual(
      expect.objectContaining({
        RBCLAW_PAIRED_TURN_ID:
          'paired-task-prep-advance:2026-04-10T00:00:00.000Z:reviewer-turn',
        RBCLAW_PAIRED_TURN_ROLE: 'reviewer',
        RBCLAW_PAIRED_TURN_INTENT: 'reviewer-turn',
        RBCLAW_PAIRED_TASK_UPDATED_AT: '2026-04-10T00:00:00.000Z',
      }),
    );
  });
...
describe('runAgentForGroup direct work directory and blocking', () => {
  it('passes direct work directory overrides into the runner when execution metadata exists', async () => {
    const group = {
      ...makeGroup(),
      folder: 'test-group',
    ...
    vi.mocked(
      ...
      workDir: '/tmp/rbclaw-test-work',
      envOverrides: {
        RBCLAW_WORK_DIR: '/tmp/paired/owner',
        RBCLAW_PAIRED_TASK_ID: 'paired-task-1',
  ...
  it('does not spawn the runner when direct work directory preparation is blocked', async () => {
    const group = {
      ...makeGroup(),
      folder: 'test-group',
      workDir: '/repo/canonical',
    ...
    vi.mocked(
      ...
      blockMessage: 'Configured channel workDir is unavailable.',
    });
    ...
    expect(outputs).toEqual([
      {
        text: 'Configured channel workDir is unavailable.',
        result: null,
      },
    ]);
    expect(
      ...
      runId: 'run-blocked-reviewer',
      status: 'failed',
      summary: 'Configured channel workDir is unavailable.',
    });
    expect(
      pairedExecutionContext.completePairedExecutionContext,
