...
function buildPathsConfig(
  ...
  return {
    ...
    storeDir: path.resolve(
      readNonEmptyText('RBCLAW_STORE_DIR') ?? path.join(projectRoot, 'store'),
    ),
    groupsDir: path.resolve(
      readNonEmptyText('RBCLAW_GROUPS_DIR') ?? path.join(projectRoot, 'groups'),
    ),
    dataDir: path.resolve(
      readNonEmptyText('RBCLAW_DATA_DIR') ?? path.join(projectRoot, 'data'),
    ),
    cacheDir: path.resolve(
      readNonEmptyText('RBCLAW_CACHE_DIR') ?? path.join(projectRoot, 'cache'),
    ),
  };
}
