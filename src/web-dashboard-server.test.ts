...
describe('assertDashboardAuthPosture', () => {
  ...
  it('warns but starts on a private / tailnet host without a token', () => {
    expect(() => assertDashboardAuthPosture('100.64.0.10', '')).not.toThrow();
...
describe('web dashboard server handler', () => {
  ...
  it('shares inbox dismiss state with overview JSON', async () => {
    ...
    const handler = createWebDashboardHandler({
      authToken: 'test-dashboard-secret',
      readStatusSnapshots: () => [],
      getTasks: () => [],
      ...
      new Request(
        `http://localhost/api/inbox/${encodeURIComponent(firstBody.inbox[0]!.id)}/actions`,
        {
          ...
          headers: {
            ...
            'x-rbclaw-dashboard-token': 'test-dashboard-secret',
          },
          body: JSON.stringify({
            action: 'dismiss',
...
describe('web dashboard server room and static routes', () => {
  it('wires room message injection dependencies', async () => {
    ...
    const handler = createWebDashboardHandler({
      ...
      hasMessage: () => false,
      publishRoomMessage: async (message) => {
    ...
    const response = await handler(
      new Request(
        `http://localhost/api/rooms/${encodeURIComponent('dc:ops')}/messages`,
        {
          ...
          headers: {
            'content-type': 'application/json',
            'x-rbclaw-dashboard-token': 'room-message-secret',
          },
          body: JSON.stringify({
