...
describe('work items', () => {
  ...
  it('stores produced work item attachments for delivery retries', () => {
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-work-item-attachment-'),
    );
    const sourcePath = path.join(tempDir, 'image.png');
    ...
    const [storedAttachment] = stored?.attachments ?? [];
    expect(storedAttachment).toMatchObject({
      name: 'image.png',
      mime: 'image/png',
    });
    expect(storedAttachment?.path).toContain(
      path.join('data', 'attachments', 'outbound', 'discord_test'),
    );
    expect(fs.existsSync(storedAttachment?.path ?? '')).toBe(true);
    expect(
      fs
        .realpathSync(storedAttachment?.path ?? '')
        .startsWith(fs.realpathSync(path.join(DATA_DIR, 'attachments'))),
    ).toBe(true);

    fs.rmSync(tempDir, { recursive: true, force: true });
    fs.rmSync(path.join(DATA_DIR, 'attachments', 'outbound', 'discord_test'), {
      recursive: true,
      force: true,
    });
  });
...
describe('work item canonical service metadata', () => {
  it('fails fast when a work item row loses canonical agent metadata after init', () => {
    const tempDir = fs.mkdtempSync('/tmp/rbclaw-work-item-strict-read-');
  ...
  it('backfills work item service ids during init on a canonical work_items schema without service_id columns', () => {
    const tempDir = fs.mkdtempSync('/tmp/rbclaw-work-items-canonical-');
    const dbPath = path.join(tempDir, 'messages.db');
