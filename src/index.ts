...
import { publishWebDashboardRoomMessage } from './web-dashboard-room-publisher.js';
...
const HANDOFF_ONLY =
  process.env.RBCLAW_HANDOFF_ONLY === '1' ||
  process.env.RBCLAW_HANDOFF_ONLY === 'true';
  ...
  startIpcWatcher({
    ...
    injectInboundMessage: async (payload) => {
      ...
      storeMessage({
        ...
        message_metadata: payload.messageMetadata ?? null,
      });
...
async function startDashboards(handles: MainRuntimeHandles): Promise<void> {
  await startUnifiedDashboard({
    ...
    serviceAgentType: CURRENT_RUNTIME_AGENT_TYPE,
  ...
  handles.webDashboardServer = startWebDashboardServer({
    ...
    publishRoomMessage: (message) =>
      publishWebDashboardRoomMessage(channels, message),
...
if (isDirectRun) {
  main().catch((err) => {
    ...
    logger.error({ err, exitCode }, 'Failed to start RBClaw');
