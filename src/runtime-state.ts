...
export function createRuntimeState(): RuntimeState {
  ...
  const loadState = (): void => {
    ...
    const recoveredMainRoomCount = repairMissingMainRoomWorkDirs(process.cwd());
    if (recoveredMainRoomCount > 0) {
      logger.warn(
        {
          recoveredMainRoomCount,
          workDir: process.cwd(),
        },
        'Recovered missing main-room workDir from the service project root',
      );
    }
