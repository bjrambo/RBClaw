...
describe('IPC message authorization', () => {
  ...
  it('normalizes raw RBClaw envelopes in authorized IPC messages', async () => {
    const sendMessage = vi.fn(async () => {});
  ...
  it('injects voice companion messages as human-equivalent inputs with replay fields', async () => {
    const sendMessage = vi.fn(async () => {});
    const injectInboundMessage = vi.fn(async () => {});
    ...
    const result = await forwardAuthorizedIpcMessage(
      {
        ...
        senderName: 'Test Voice Companion',
        treatAsHuman: true,
        sourceKind: 'voice_companion',
        nonce: 'voice-nonce-001',
        timestamp,
      },
      'other-group',
      false,
    ...
    expect(injectInboundMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        sender: 'voice-companion',
        senderName: 'Test Voice Companion',
        messageId: 'voice-voice-nonce-001',
        timestamp,
        treatAsHuman: true,
        sourceKind: 'voice_companion',
  ...
  it('blocks voice companion messages without valid replay fields', async () => {
    const sendMessage = vi.fn(async () => {});
    const injectInboundMessage = vi.fn(async () => {});
  ...
  it('blocks high-risk voice companion requests at IPC ingress', async () => {
    const sendMessage = vi.fn(async () => {});
    let blockedText = '';
    const injectInboundMessage = vi.fn(async (payload: { text: string }) => {
      blockedText = payload.text;
    });
    const timestamp = new Date().toISOString();
    ...
    expect(injectInboundMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        messageId: 'voice-blocked-voice-nonce-002',
        timestamp,
        treatAsHuman: false,
  ...
  it('blocks destructive git cleanup wording from voice companion IPC', async () => {
    const sendMessage = vi.fn(async () => {});
    let blockedText = '';
    const injectInboundMessage = vi.fn(async (payload: { text: string }) => {
      blockedText = payload.text;
    ...
    const result = await forwardAuthorizedIpcMessage(
      {
        type: 'inject_inbound_message',
        chatJid: 'other@g.us',
        text: 'reset rebase force 정리 작업 진행해',
        senderName: 'Test Voice Companion',
        treatAsHuman: true,
        sourceKind: 'voice_companion',
        nonce: 'voice-nonce-git-cleanup',
        timestamp,
      },
      'other-group',
      false,
    ...
    expect(blockedText).toContain('local_confirm_click');
  });

  it('does not trust legacy typed-pin high-risk voice confirmation metadata', async () => {
    const sendMessage = vi.fn(async () => {});
    let blockedText = '';
    const injectInboundMessage = vi.fn(async (payload: { text: string }) => {
      blockedText = payload.text;
    ...
    const result = await forwardAuthorizedIpcMessage(
      {
        ...
        treatAsHuman: true,
        sourceKind: 'voice_companion',
        nonce: 'voice-nonce-003',
        timestamp,
        approvalLevel: 'high_risk_confirmed',
        approvalMethod: 'typed_pin',
      },
      'other-group',
      false,
      groups,
    ...
    expect(injectInboundMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        sender: 'voice-companion-gate',
        senderName: 'Voice Companion Gate',
        messageId: 'voice-blocked-voice-nonce-003',
        timestamp,
        treatAsHuman: false,
        sourceKind: 'ipc_injected_bot',
        messageMetadata: JSON.stringify({
  ...
  it('allows locally confirmed high-risk voice companion IPC', async () => {
    const sendMessage = vi.fn(async () => {});
    const injectInboundMessage = vi.fn(async () => {});
    const timestamp = new Date().toISOString();
    ...
    const result = await forwardAuthorizedIpcMessage(
      {
        ...
        treatAsHuman: true,
        sourceKind: 'voice_companion',
        nonce: 'voice-nonce-confirmed-git-cleanup',
        timestamp,
        approvalLevel: 'high_risk_confirmed',
        approvalMethod: 'local_confirm_click',
      },
      'other-group',
      false,
      groups,
    ...
    expect(injectInboundMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        text: 'reset rebase force 정리 작업 진행해',
        senderName: 'Test Voice Companion',
        messageId: 'voice-voice-nonce-confirmed-git-cleanup',
        timestamp,
        treatAsHuman: true,
        sourceKind: 'voice_companion',
        messageMetadata: JSON.stringify({
