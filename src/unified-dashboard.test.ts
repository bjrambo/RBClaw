...
import {
  ...
  findCodexUsageSnapshot,
  formatStatusHeader,
  getDashboardDuplicateCleanupIntervalMs,
  renderUsageTable,
  resolveDashboardEntryAgentType,
...
describe('resolveDashboardEntryAgentType', () => {
  it('uses the room entry agent type before the service snapshot agent type', () => {
    expect(
      resolveDashboardEntryAgentType({
        entryAgentType: 'codex',
        snapshotAgentType: 'claude-code',
      }),
    ).toBe('codex');
  });

  it('falls back to the snapshot agent type when the entry is missing it', () => {
    expect(
      resolveDashboardEntryAgentType({
        entryAgentType: undefined,
        snapshotAgentType: 'codex',
      }),
    ).toBe('codex');
  });
});

describe('findCodexUsageSnapshot', () => {
  it('uses unified service snapshots that contain Codex usage rows', () => {
    const snapshot = findCodexUsageSnapshot([
      {
        serviceId: 'zinna',
    ...
    expect(snapshot?.serviceId).toBe('zinna');
  });
});
