...
describe('web dashboard room routes', () => {
  ...
  it('serves registered room timelines even when status snapshots are quiet', async () => {
    const messages: NewMessage[] = [
      {
        id: 'quiet-msg',
        chat_jid: 'dc:quiet',
    ...
    const response = handleRoomTimelineRoute({
      url: new URL(
        `http://localhost/api/rooms/${encodeURIComponent('dc:quiet')}/timeline`,
      ),
      request: new Request('http://localhost'),
      jsonResponse,
      ...roomDeps({
        ...
        loadRecentChatMessages: () => messages,
      }),
    });
    ...
    expect(body.messages).toEqual([
      expect.objectContaining({ content: '조용한 방 확인' }),
    ]);
  });
});
