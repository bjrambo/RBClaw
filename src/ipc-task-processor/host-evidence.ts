...
export async function handleHostEvidenceRequest(
  ...
  if (data.action === 'rbclaw_room_runtime') {
...
function handleRoomRuntimeReportRequest(
  ...
  if (!data.chatJid) {
    writeHostEvidenceResponse(sourceGroup, {
      requestId: data.requestId!,
      ok: false,
      action: 'rbclaw_room_runtime',
      command: 'internal:rbclaw_room_runtime',
      stdout: '',
      stderr: '',
      exitCode: 1,
      error: 'Missing chatJid for rbclaw_room_runtime request',
    });
    logger.warn(
      { sourceGroup, requestId: data.requestId },
      'Rejected rbclaw_room_runtime request without chatJid',
    );
    return;
  }
  ...
  if (!deps.getRoomRuntimeReport) {
    ...
    logger.warn(
      ...
      'Rejected rbclaw_room_runtime request because runtime reporter is unavailable',
    );
    return;
  }
