...
vi.mock('./config.js', () => ({
  DATA_DIR: '/tmp/rbclaw-compact-refresh-test',
}));
...
const refreshDir = '/tmp/rbclaw-compact-refresh-test/compact-refresh';
...
describe('compact-refresh', () => {
  ...
  it('applies a one-shot compact refresh only for owner and reviewer roles', () => {
    ...
    expect(applied?.prompt).toContain('RBClaw compact refresh');
