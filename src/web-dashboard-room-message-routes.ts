...
export function createRoomMessageIdCache(
  ...
  const remember = ((id: string): boolean => {
    ...
    return true;
  }) as RoomMessageIdRememberer;

  remember.release = (id: string): void => {
    if (!idSet.delete(id)) return;
...
export async function handleRoomMessageRoute({
  ...
  if (!loadRoomBindings || !enqueueMessageCheck || !publishRoomMessage) {
    return jsonResponse(
  ...
  const cacheKey = `${messageRoomJid}:${id}`;
  if (!rememberRoomMessageId(cacheKey)) {
    return jsonResponse({ ok: true, id, queued: false, duplicate: true });
  }

  const senderName = body.nickname ?? 'Web Dashboard';
  try {
    await publishRoomMessage({
      chatJid: messageRoomJid,
      senderName,
      text: body.text,
    });
  } catch {
    rememberRoomMessageId.release(cacheKey);
    return jsonResponse(
      { error: 'Failed to publish room message to Discord' },
      { status: 502 },
    );
  }
