...
export function repairMissingMainRoomWorkDirsInDatabase(
  database: Database,
  workDir: string,
): number {
  if (!workDir.trim()) {
    throw new Error('Main room recovery requires a non-empty workDir.');
  }
  ...
  const result = database
    ...
    .run(workDir, new Date().toISOString());
