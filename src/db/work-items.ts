...
function outboundAttachmentName(
...
function normalizeOutboundAttachmentsForWorkItem(input: {
  groupFolder: string;
  attachments?: OutboundAttachment[];
  now: string;
}): {
  ...
  if (!input.attachments?.length) return { failures: [] };

  const batchDir = path.join(
    DATA_DIR,
    'attachments',
    'outbound',
    sanitizePathSegment(input.groupFolder, 'group'),
    `${input.now.replace(/[^0-9TZ.-]+/g, '-')}-${randomUUID().slice(0, 8)}`,
  );
  const normalized: OutboundAttachment[] = [];
  const failures: Array<{ name: string; reason: string }> = [];

  input.attachments.forEach((attachment, index) => {
    const name = attachment.name
      ? path.basename(attachment.name)
      : path.basename(attachment.path) || `attachment-${index + 1}`;
    try {
      if (!path.isAbsolute(attachment.path)) {
        failures.push({ name, reason: 'not-absolute' });
        return;
      }
      ...
      if (!fs.statSync(sourcePath).isFile()) {
        failures.push({ name, reason: 'not-file' });
        return;
      }
      ...
      const targetPath = path.join(
        batchDir,
        outboundAttachmentName(attachment, sourcePath, index),
      );
      ...
      failures.push({ name, reason: 'copy-failed' });
    }
  });

  return {
    attachments: normalized.length > 0 ? normalized : undefined,
    failures,
  };
}

function appendAttachmentCopyFailures(
  resultPayload: string,
  failures: Array<{ name: string; reason: string }>,
): string {
  if (failures.length === 0) return resultPayload;
  const lines = failures.map(
    (failure) => `- ${failure.name}: ${failure.reason}`,
  );
  return `${resultPayload.trimEnd()}\n\n[첨부 전송 실패]\n${lines.join('\n')}`;
}
...
export function createProducedWorkItemInDatabase(
  ...
  const normalizedAttachments = normalizeOutboundAttachmentsForWorkItem({
    groupFolder: input.group_folder,
    attachments: input.attachments,
    now,
  });
  const resultPayload = appendAttachmentCopyFailures(
    input.result_payload,
    normalizedAttachments.failures,
  );
  ...
  return database.transaction(() => {
    ...
    database
      ...
      .run(
        ...
        input.end_seq,
        resultPayload,
        serializeAttachmentPayload(normalizedAttachments.attachments),
        now,
        now,
      );
