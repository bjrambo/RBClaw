    ...
    database
      .prepare(
        `INSERT INTO messages (
         id, chat_jid, sender, sender_name, content, timestamp, seq, is_from_me, is_bot_message, message_source_kind, message_metadata
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
export function getNewMessagesFromDatabase(
  ...
  const sql = `
    SELECT * FROM (
      SELECT id, chat_jid, sender, sender_name, content, timestamp, is_from_me, is_bot_message, message_source_kind, message_metadata
      FROM messages
...
export function getNewMessagesBySeqFromDatabase(
  ...
  const placeholders = jids.map(() => '?').join(',');
  const sql = `
    SELECT id, chat_jid, sender, sender_name, content, timestamp, seq, is_from_me, is_bot_message, message_source_kind, message_metadata
    FROM messages
  ...
  const sql = `
    WITH ranked AS (
      SELECT id, chat_jid, sender, sender_name, content, timestamp,
             is_from_me, is_bot_message, message_source_kind, message_metadata,
