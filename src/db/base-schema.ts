...
function applyScheduledTaskTables(database: Database): void {
  database.exec(`
    CREATE TABLE IF NOT EXISTS scheduled_tasks (
      ...
      paired_task_id TEXT,
      external_wait_ref TEXT,
      watcher_dedup_key TEXT,
      terminal_event_applied_at TEXT,
...
function applyPairedTaskTables(database: Database): void {
  database.exec(`
    CREATE TABLE IF NOT EXISTS paired_tasks (
      ...
      last_blocker_class TEXT,
      resume_at TEXT,
      supervisor_state TEXT NOT NULL DEFAULT 'runnable',
      supervisor_state_changed_at TEXT,
      last_arbiter_directive_fingerprint TEXT,
      last_arbiter_directive_json TEXT,
      ...
      review_phase TEXT NOT NULL DEFAULT 'implementation',
      ...
      CHECK (supervisor_state IN ('runnable', 'waiting_retry', 'waiting_external', 'waiting_user', 'parked', 'terminal')),
      CHECK (length(trim(work_dir)) > 0),
