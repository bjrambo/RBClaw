...
describe('validateOutboundAttachments', () => {
  it('accepts real image files under default attachment directories', () => {
    const dir = makeTempDir(os.tmpdir(), 'rbclaw-attachment-');
  ...
  it('accepts direct RBClaw screenshot files under the temp directory', () => {
    const imagePath = path.join(
      os.tmpdir(),
      `rbclaw-room-mobile-list-${Date.now()}.png`,
    );
  ...
  it('accepts supported non-image media and document files', () => {
    const dir = makeTempDir(os.tmpdir(), 'rbclaw-media-');
...
describe('validateOutboundAttachments policy checks', () => {
  function useIsolatedDefaultTempDir(): void {
    const tempDir = makeTempDir(os.tmpdir(), 'rbclaw-policy-temp-');
  ...
  it('requires workspace paths to be explicitly allowlisted', () => {
    ...
    const dir = makeTempDir(process.cwd(), '.rbclaw-attachment-');
  ...
  it('accepts user-configured attachment directories from env', () => {
    const unusedCommaDir = makeTempDir(process.cwd(), '.rbclaw-unused-images-');
    const unusedDelimiterDir = makeTempDir(
      process.cwd(),
      '.rbclaw-more-unused-images-',
    );
    const dir = makeTempDir(process.cwd(), '.rbclaw-user-images-');
    const imagePath = writeFile(dir, 'manual-shot.png', ONE_PIXEL_PNG);
    vi.stubEnv(
      'RBCLAW_ATTACHMENT_ALLOWED_DIRS',
  ...
  it('rejects symlink attempts that escape the allowed directory', () => {
    useIsolatedDefaultTempDir();
    const workspaceDir = makeTempDir(process.cwd(), '.rbclaw-attachment-');
    const targetPath = writeFile(
      workspaceDir,
      'secret-shot.png',
      ONE_PIXEL_PNG,
    );
    const tmpDir = makeTempDir(os.tmpdir(), 'rbclaw-attachment-');
  ...
  it('rejects symlink attempts that escape a user-configured directory', () => {
    useIsolatedDefaultTempDir();
    const allowedDir = makeTempDir(process.cwd(), '.rbclaw-user-images-');
    const secretDir = makeTempDir(process.cwd(), '.rbclaw-secret-images-');
    const targetPath = writeFile(secretDir, 'secret-shot.png', ONE_PIXEL_PNG);
    const linkPath = path.join(allowedDir, 'linked-shot.png');
    fs.symlinkSync(targetPath, linkPath);
    vi.stubEnv('RBCLAW_ATTACHMENT_ALLOWED_DIRS', allowedDir);
