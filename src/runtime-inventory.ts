...
function summarizeTomlMcpConfig(content: string): McpConfigSummary {
  ...
  return {
    rbclawConfigured: /^\s*\[mcp_servers\.rbclaw(?:\.[^\]]+)?\]/m.test(content),
...
function summarizeJsonMcpConfig(content: string): McpConfigSummary | null {
  ...
  return {
    rbclawConfigured: names.includes('rbclaw'),
...
export function getRuntimeInventory(
  ...
  const rbclawRunnerSkills = readSkillDir(
    'RBClaw runner skills',
    runnerSkillDir,
  );
  ...
  return {
    ...
    rbclaw: {
      runnerSkillDir: rbclawRunnerSkills,
      mcpServer: pathSnapshot('RBClaw IPC MCP server', mcpServerPath),
    },
  };
}
