...
vi.mock('./config.js', () => ({
  DATA_DIR: '/tmp/rbclaw-codex-rot-data',
}));
...
describe('codex-token-rotation d7 ≥ 100% auto-skip', () => {
  ...
  beforeEach(() => {
    vi.resetModules();
    tempHome = fs.mkdtempSync(path.join('/tmp', 'rbclaw-codex-rot-'));
  ...
  it('warns when Codex rotation state cannot be persisted', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      expect.objectContaining({
        stateFile: '/tmp/rbclaw-codex-rot-data/codex-rotation-state.json',
...
describe('codex-token-rotation single-account fallback', () => {
  ...
  beforeEach(() => {
    vi.resetModules();
    tempHome = fs.mkdtempSync(path.join('/tmp', 'rbclaw-codex-fallback-'));
    process.env.CODEX_ROT_TEST_HOME = tempHome;
  });
