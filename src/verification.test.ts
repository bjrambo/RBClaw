...
describe('verification helpers', () => {
  ...
  it('builds deterministic commands for each profile', () => {
    const repoDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-verification-'),
    );
  ...
  it('prefers lint:ci and lint:check before plain lint', () => {
    const repoDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-verification-lint-'),
    );
    fs.writeFileSync(
  ...
  it('selects the workspace package manager for verification commands', () => {
    const repoDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-verification-pnpm-'),
    );
    fs.writeFileSync(
      path.join(repoDir, 'package.json'),
  ...
  it('builds nested pnpm verification commands without tripping corepack project specs', () => {
    const parentDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-verification-corepack-parent-'),
    );
    fs.writeFileSync(
...
describe('verification snapshots', () => {
  it('computes a stable snapshot over the readable workspace inputs', () => {
    const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rbclaw-snapshot-'));
  ...
  it('ignores runtime state directories and local backup folders', () => {
    ...
    fs.mkdirSync(path.join(repoDir, '.rbclaw-reviewer-runtime'), {
      recursive: true,
    });
    ...
    fs.writeFileSync(
      path.join(repoDir, '.rbclaw-reviewer-runtime', 'runtime.json'),
      '{"ok":true}\n',
    );
    fs.writeFileSync(
...
describe('runVerificationRequest direct execution', () => {
  it('backfills legacy node_modules state before verification', async () => {
    const repoDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-verification-legacy-'),
    );
  ...
  it('runs typecheck directly on the host for non-build profiles', async () => {
    const repoDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-verification-direct-'),
    );
    fs.mkdirSync(path.join(repoDir, 'node_modules', '.bin'), {
      recursive: true,
    });
  ...
  it('runs lint verification using the preferred lint script', async () => {
    const repoDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-verification-direct-lint-'),
    );
    fs.mkdirSync(path.join(repoDir, 'node_modules', '.bin'), {
      recursive: true,
    });
...
describe('runVerificationRequest workspace guards', () => {
  it('does not leak host secrets into direct verification commands', async () => {
    const repoDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-verification-direct-secret-'),
    );
    fs.mkdirSync(path.join(repoDir, 'node_modules', '.bin'), {
      recursive: true,
    });
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        scripts: {
          typecheck:
            'node -e "process.stdout.write(process.env.RBCLAW_VERIFICATION_SECRET || \'missing\')"',
        },
      }),
    );
    fs.writeFileSync(path.join(repoDir, 'bun.lock'), '');
    ...
    const previousSecret = process.env.RBCLAW_VERIFICATION_SECRET;
    process.env.RBCLAW_VERIFICATION_SECRET = 'top-secret';
    ...
    try {
      ...
      if (previousSecret == null) {
        delete process.env.RBCLAW_VERIFICATION_SECRET;
      } else {
        process.env.RBCLAW_VERIFICATION_SECRET = previousSecret;
      }
    }
  });
  ...
  it('fails direct verification if the command mutates the workspace', async () => {
    const repoDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-verification-direct-mutate-'),
    );
    fs.mkdirSync(path.join(repoDir, 'node_modules', '.bin'), {
      recursive: true,
    });
  ...
  it('allows build verification to write excluded output directories', async () => {
    const repoDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-verification-build-direct-'),
    );
    fs.mkdirSync(path.join(repoDir, 'node_modules', '.bin'), {
      recursive: true,
    });
