...
describe('settings-store Codex features', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rbclaw-settings-'));
    previousCwd = process.cwd();
    previousCodexGoals = process.env.CODEX_GOALS;
    previousHome = process.env.HOME;
    previousSettingsHome = process.env.RBCLAW_SETTINGS_HOME;
    previousFetch = globalThis.fetch;
    delete process.env.CODEX_GOALS;
    process.env.HOME = tempDir;
    process.env.RBCLAW_SETTINGS_HOME = tempDir;
  ...
  afterEach(() => {
    ...
    if (previousSettingsHome === undefined) {
      delete process.env.RBCLAW_SETTINGS_HOME;
    } else {
      process.env.RBCLAW_SETTINGS_HOME = previousSettingsHome;
    }
