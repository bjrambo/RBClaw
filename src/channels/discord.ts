...
function replaceConfiguredMentionAlias(text: string): string {
  const configuredAlias = getEnv(DISCORD_MENTION_ALIAS_KEY)?.trim();
  if (!configuredAlias) return text;

  const alias = configuredAlias.replace(/^@/, '');
  if (!alias || /[\s<>@]/u.test(alias)) {
    logger.warn(
      { envKey: DISCORD_MENTION_ALIAS_KEY },
      'Discord mention alias is invalid; leaving outbound text unchanged',
    );
    return text;
  }

  const mentionAlias = `@${alias}`;
  if (!text.includes(mentionAlias)) return text;

  const userId = getEnv(DISCORD_MENTION_USER_ID_KEY)?.trim();
  if (!userId) return text;

  if (!DISCORD_USER_ID_PATTERN.test(userId)) {
    logger.warn(
      { envKey: DISCORD_MENTION_USER_ID_KEY },
      'Discord mention user ID is invalid; leaving alias unchanged',
    );
    return text;
  }

  return text.replaceAll(mentionAlias, `<@${userId}>`);
}

function attachmentFailureNotice(
  rejected: Array<{ path: string; reason: string }>,
): string {
  if (rejected.length === 0) return '';
  const lines = rejected.map((item) => {
    const name = path.basename(item.path) || item.path;
    return `- ${name}: ${item.reason}`;
  });
  return `[첨부 전송 실패]\n${lines.join('\n')}`;
}

function transientAttachmentRejections(
  rejected: Array<{ path: string; reason: string }>,
): Array<{ path: string; reason: string }> {
  return rejected.filter((item) =>
    TRANSIENT_ATTACHMENT_REJECTION_REASONS.has(item.reason),
  );
}
...
export class DiscordChannel implements Channel {
  ...
  async sendMessage(
    ...
    try {
      ...
      const transientRejected = transientAttachmentRejections(
        validation.rejected,
      );
      if (transientRejected.length > 0) {
        throw new Error(
          `Discord attachment validation failed: ${JSON.stringify(
            transientRejected,
          )}`,
        );
      }
      const permanentRejected = validation.rejected.filter(
        (item) => !TRANSIENT_ATTACHMENT_REJECTION_REASONS.has(item.reason),
      );
      ...
      let cleaned = outbound.cleanText
        ...
        .trim();
      const failureNotice = attachmentFailureNotice(permanentRejected);
      if (failureNotice) {
        cleaned = [cleaned, failureNotice].filter(Boolean).join('\n\n');
      }

      // Convert the configured local alias to a Discord mention.
