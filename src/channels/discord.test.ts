...
vi.mock('../config.js', () => ({
  ...
  CACHE_DIR: '/tmp/rbclaw-test-cache',
...
function createTempPng(name = 'image.png'): { dir: string; filePath: string } {
  ...
  const dir = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), 'rbclaw-discord-image-')),
  );
...
describe('sendMessage', () => {
  ...
  it('converts a configured alias to the Discord user mention', async () => {
    vi.mocked(getEnv).mockImplementation((key) => {
      if (key === 'DISCORD_MENTION_ALIAS') return 'Operator';
      if (key === 'DISCORD_MENTION_USER_ID') return '111111111111111111';
      return undefined;
    });
    const opts = createTestOpts();
    const channel = new DiscordChannel('test-token', opts);
    ...
    await channel.sendMessage('dc:1234567890123456', '@Operator 확인 완료');

    expect(mockChannel.send).toHaveBeenCalledWith({
      content: '<@111111111111111111> 확인 완료',
      files: undefined,
      flags: 1 << 2,
    });
  });

  it('leaves an alias unchanged when the Discord user ID is not configured', async () => {
    vi.mocked(getEnv).mockImplementation((key) =>
      key === 'DISCORD_MENTION_ALIAS' ? 'Operator' : undefined,
    );
    const opts = createTestOpts();
    const channel = new DiscordChannel('test-token', opts);
    await channel.connect();
    ...
    expect(mockChannel.send).toHaveBeenCalledWith({
      content: '@Operator 확인 완료',
      files: undefined,
      flags: 1 << 2,
    });
  });

  it('rejects an invalid configured Discord user ID for mentions', async () => {
    vi.mocked(getEnv).mockImplementation((key) => {
      if (key === 'DISCORD_MENTION_ALIAS') return 'Operator';
      if (key === 'DISCORD_MENTION_USER_ID') return 'not-a-discord-id';
      return undefined;
    });
    const opts = createTestOpts();
    const channel = new DiscordChannel('test-token', opts);
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      { envKey: 'DISCORD_MENTION_USER_ID' },
  ...
  it('surfaces permanently rejected attachments in the Discord message', async () => {
    const opts = createTestOpts();
    const channel = new DiscordChannel('test-token', opts);
    await channel.connect();
    const mockChannel = {
    ...
    await channel.sendMessage('dc:1234567890123456', '검증 캡처입니다.', {
      attachments: [
        {
          path: '/tmp/not-supported.svg',
          name: 'not-supported.svg',
          mime: 'image/svg+xml',
        },
      ],
    });
    ...
    expect(mockChannel.send).toHaveBeenCalledWith({
      content:
        '검증 캡처입니다.\n\n[첨부 전송 실패]\n- not-supported.svg: unsupported-extension',
      files: undefined,
      flags: 1 << 2,
    });
  });
