...
describe('DiscordChannel structured output', () => {
  it('normalizes raw RBClaw JSON and sends direct temp images as files', async () => {
  ...
  it('normalizes status-prefixed RBClaw JSON and keeps the status line visible', async () => {
    const channel = new DiscordChannel('test-token', createTestOpts());
    await channel.connect();
    const filePath = path.join(
      os.tmpdir(),
      `rbclaw-discord-image-status-${Date.now()}.png`,
    );
    fs.writeFileSync(filePath, ONE_PIXEL_PNG);
    tempFiles.push(filePath);
    const mockChannel = {
...
\`\`\``,
  ...
  it('normalizes markdown images and sends them as files', async () => {
    ...
    const filePath = path.join(
      os.tmpdir(),
      `rbclaw-discord-markdown-image-${Date.now()}.png`,
    );
    fs.writeFileSync(filePath, ONE_PIXEL_PNG);
    tempFiles.push(filePath);
    const mockChannel = {
