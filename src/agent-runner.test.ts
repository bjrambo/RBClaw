...
vi.mock('fs', async () => {
  ...
  const realpathSync = vi.fn((p: fs.PathLike) =>
    String(p),
  ) as unknown as typeof fs.realpathSync;
  realpathSync.native = vi.fn((p: fs.PathLike) =>
    String(p),
  ...
  return {
    ...
    default: {
      ...
      existsSync: vi.fn((p: string) => {
        if (
          typeof p === 'string' &&
          (p.includes('dist/index.js') ||
            p === '/usr/bin/unshare' ||
            p.endsWith('/scripts/run-agent-filesystem-sandbox.sh'))
        ) {
          return true;
        }
        return false;
...
function isRequiredRunnerPath(value: string): boolean {
  return (
    value.includes('dist/index.js') ||
    value.includes('dist/ipc-mcp-stdio.js') ||
    value === '/usr/bin/unshare' ||
    value.endsWith('/scripts/run-agent-filesystem-sandbox.sh')
  );
}
...
function mockCodexLeaseEnvironment(releaseLease: () => void) {
  return vi
    ...
    .mockReturnValueOnce({
      env: {
        RBCLAW_IPC_DIR: '/tmp/rbclaw-test-data/ipc/test-group',
        RBCLAW_WORK_DIR: '/tmp',
      },
      groupDir: '/tmp/rbclaw-test-groups/test-group',
      runnerDir: '/tmp/rbclaw-test-runners/codex-runner',
      codexSessionAuth: {
...
describe('agent-runner environment wiring', () => {
  ...
  it('fails closed when the channel has no configured workDir', async () => {
    const result = await runAgentProcess(
      { ...testGroup, workDir: undefined },
      testInput,
      () => {},
    );
    ...
    expect(result).toEqual({
      status: 'error',
      result: null,
      error: 'Channel workDir is required; group-folder fallback is disabled.',
    });
  ...
  it('passes the actual chat JID into codex runner MCP env', async () => {
    ...
    vi.mocked(fs.existsSync).mockImplementation((p: fs.PathLike) => {
      const str = String(p);
      return isRequiredRunnerPath(str) || str.endsWith('/.codex/config.toml');
    });
    ...
    const spawnEnv = vi.mocked(spawn).mock.calls[0]?.[2]?.env as
      ...
      | undefined;
    expect(spawnEnv?.RBCLAW_CHAT_JID).toBe(testInput.chatJid);
    ...
    expect(String(tomlWrite?.[1])).toContain(
      `RBCLAW_CHAT_JID = ${JSON.stringify(testInput.chatJid)}`,
    );
  });
  ...
  it('uses host IPC and workdir in unsafe host paired mode reviewer sessions', async () => {
    ...
    const resultPromise = runAgentProcess(
      ...
      async () => {},
      {
        ...
        RBCLAW_WORK_DIR: '/tmp/paired/task-1/reviewer',
      },
    );
    ...
    expect(prepareReadonlySessionEnvironmentSpy).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        ipcDir: '/tmp/rbclaw-test-data/ipc/test-group',
        hostIpcDir: '/tmp/rbclaw-test-data/ipc/test-group',
...
describe('agent-runner task and config environment wiring', () => {
  ...
  it('isolates IPC and session directories for isolated scheduled tasks', async () => {
    ...
    const spawnEnv = vi.mocked(spawn).mock.calls.at(-1)?.[2]?.env as
      | Record<string, string>
      | undefined;
    expect(spawnEnv?.RBCLAW_IPC_DIR).toBe(
      '/tmp/rbclaw-test-data/ipc/test-group/tasks/task-123',
    );
    expect(spawnEnv?.RBCLAW_HOST_IPC_DIR).toBe(
      '/tmp/rbclaw-test-data/ipc/test-group',
    );
    expect(spawnEnv?.CLAUDE_CONFIG_DIR).toBe(
      '/tmp/rbclaw-test-data/sessions/test-group/services/claude/tasks/task-123/.claude',
    );
  });

  it('writes RBCLAW_HOST_IPC_DIR into task-scoped codex MCP config', async () => {
    vi.useRealTimers();
    fakeProc = createFakeProcess();
    ...
    expect(String(tomlWrite?.[1])).toContain(
      'RBCLAW_IPC_DIR = "/tmp/rbclaw-test-data/ipc/test-group/tasks/task-codex-watch"',
    );
    expect(String(tomlWrite?.[1])).toContain(
      'RBCLAW_HOST_IPC_DIR = "/tmp/rbclaw-test-data/ipc/test-group"',
    );
  });
  ...
  it('keeps shared session history for group-context task runtimes while isolating IPC', async () => {
    ...
    const spawnEnv = vi.mocked(spawn).mock.calls.at(-1)?.[2]?.env as
      ...
      | undefined;
    expect(spawnEnv?.RBCLAW_IPC_DIR).toBe(
      '/tmp/rbclaw-test-data/ipc/test-group/tasks/task-watch-group',
    );
    expect(spawnEnv?.RBCLAW_HOST_IPC_DIR).toBe(
      '/tmp/rbclaw-test-data/ipc/test-group',
    );
    expect(spawnEnv?.CLAUDE_CONFIG_DIR).toBe(
      '/tmp/rbclaw-test-data/sessions/test-group/services/claude/.claude',
    );
  });
  ...
  it('merges a per-group codex config overlay before injecting managed MCP servers', async () => {
    vi.useRealTimers();
    fakeProc = createFakeProcess();
    const overlayPath = '/tmp/rbclaw-test-groups/test-group/.codex/config.toml';
    const sessionConfigPath =
      '/tmp/rbclaw-test-data/sessions/test-group/services/claude/.codex/config.toml';
    let sessionToml = `model = "gpt-5.4"\n`;

    vi.mocked(fs.existsSync).mockImplementation((p: fs.PathLike) => {
      const str = String(p);
      return (
        isRequiredRunnerPath(str) ||
