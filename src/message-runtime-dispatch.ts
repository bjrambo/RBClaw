...
function advancePastSessionCommand(args: {
  chatJid: string;
  commandMessage: NewMessage;
  lastAgentTimestamps: Record<string, string>;
  saveState: () => void;
}): void {
  advanceLastAgentCursor(
    ...
    args.saveState,
    args.chatJid,
    args.commandMessage.seq ?? args.commandMessage.timestamp,
  );
}
...
export async function processLoopGroupMessages(args: {
  ...
  if (loopCmdMsg) {
    ...
    if (loopCommand === '/stop' && isAllowedSessionCommand) {
      const killed = args.killProcess();
      completePairedTaskAsStopped(chatJid);
      advancePastSessionCommand({
        chatJid,
        commandMessage: loopCmdMsg,
        lastAgentTimestamps: args.lastAgentTimestamps,
        saveState: args.saveState,
      });
