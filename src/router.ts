...
export function formatMessages(
  ...
  const lines = messages.map((m) => {
    ...
    const sourceAttrs =
      m.message_source_kind === 'voice_companion'
        ? ` source_kind="voice_companion"${formatVoiceMetadataAttrs(
            m.message_metadata,
          )}`
        : '';
    return `<message sender="${escapeXml(m.sender_name)}" time="${escapeXml(displayTime)}"${sourceAttrs}>${escapeXml(m.content)}</message>`;
  });
...
function formatVoiceMetadataAttrs(metadata: string | null | undefined): string {
  if (!metadata) return '';
  try {
    const parsed = JSON.parse(metadata) as {
      approvalLevel?: unknown;
      approvalMethod?: unknown;
    };
    const approvalLevel =
      typeof parsed.approvalLevel === 'string' ? parsed.approvalLevel : null;
    const approvalMethod =
      typeof parsed.approvalMethod === 'string' ? parsed.approvalMethod : null;
    return [
      approvalLevel ? ` approval_level="${escapeXml(approvalLevel)}"` : '',
      approvalMethod ? ` approval_method="${escapeXml(approvalMethod)}"` : '',
    ].join('');
  } catch {
    return '';
  }
