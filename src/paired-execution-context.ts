...
function resolveRoomWorkDir(group: RegisteredGroup): string | null {
  if (!group.workDir) {
    return null;
  }
  return resolveDirectWorkDir(group.workDir);
}
...
export function resolveOwnerTaskForHumanMessage(args: {
  ...
  const canonicalWorkDir = resolveRoomWorkDir(args.group);
  const existing =
...
function ensureActiveTask(
  ...
  const canonicalWorkDir = resolveRoomWorkDir(group);
  if (!canonicalWorkDir) {
    return null;
  }
...
function prepareOwnerRoleExecution(args: {
  ...
  return { workDir: resolveDirectWorkDir(latestTask.work_dir) };
}
...
function prepareReadonlyRoleExecution(args: {
  ...
  if (refreshedTask.status === args.pendingStatus) {
    transitionPairedTaskStatus({
      taskId: latestTask.id,
...
function buildRoleEnvOverrides(args: {
  ...
  const { group, roomRoleContext, workDir } = args;
  const envOverrides: Record<string, string> = {
    RBCLAW_PAIRED_TASK_ID: args.taskId,
    RBCLAW_PAIRED_ROLE: roomRoleContext.role,
  };
  const unsafeHostPairedMode = isUnsafeHostPairedModeEnabled();

  envOverrides.RBCLAW_WORK_DIR = workDir;
...
export function preparePairedExecutionContext(args: {
  ...
  let workDir = resolveDirectWorkDir(latestTask.work_dir);
  ...
  if (roomRoleContext.role === 'owner') {
    ...
    workDir = ownerPreparation.workDir;
    ...
    workDir = reviewerPreparation.workDir;
    blockMessage = reviewerPreparation.blockMessage;
  } else if (roomRoleContext.role === 'arbiter') {
    // Arbiter uses the same read-only work directory as reviewer.
