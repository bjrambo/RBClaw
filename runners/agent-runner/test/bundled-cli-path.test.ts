...
describe('resolveBundledClaudeCodeExecutable', () => {
  const origEnv = process.env.RBCLAW_CLAUDE_CLI_PATH;

  afterEach(() => {
    if (origEnv === undefined) {
      delete process.env.RBCLAW_CLAUDE_CLI_PATH;
    } else {
      process.env.RBCLAW_CLAUDE_CLI_PATH = origEnv;
    }
  });

  it('honors RBCLAW_CLAUDE_CLI_PATH when the file exists', () => {
    ...
    const result = resolveBundledClaudeCodeExecutable({
      env: { RBCLAW_CLAUDE_CLI_PATH: fake },
      existsSync,
      platform: 'linux',
      arch: 'x64',
  ...
  it('throws when env override points at a non-existent file', () => {
    ...
    expect(() =>
      resolveBundledClaudeCodeExecutable({
        env: { RBCLAW_CLAUDE_CLI_PATH: '/does/not/exist' },
        existsSync,
        platform: 'linux',
        arch: 'x64',
        resolvePackageDir: () => null,
      }),
    ).toThrow(/RBCLAW_CLAUDE_CLI_PATH/);
  });
  ...
  it('uses a dedicated glm-code launcher for glm-code agents', () => {
    ...
    const result = resolveClaudeCompatibleExecutable({
      agentType: 'glm-code',
      env: { RBCLAW_GLM_CODE_CLI_PATH: fake },
      existsSync: (p) => p === fake,
      resolveCommand: () => null,
      knownGlmCodePaths: [],
      platform: 'linux',
  ...
  it('keeps Claude Code agents on the bundled Claude binary even when glm-code is configured', () => {
    ...
    const result = resolveClaudeCompatibleExecutable({
      ...
      env: { RBCLAW_GLM_CODE_CLI_PATH: '/opt/glm-code/bin/glm-code' },
