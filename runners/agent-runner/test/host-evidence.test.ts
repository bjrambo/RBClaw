...
describe('runner host evidence helpers', () => {
  ...
  it('reads and removes the host evidence response file', async () => {
    const ipcDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rbclaw-host-ipc-'));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        command: 'systemctl --user show rbclaw',
  ...
  it('formats the response into a compact MCP-friendly text block', () => {
    const text = formatHostEvidenceResponse({
      ...
      command: 'journalctl --user -u rbclaw --no-pager -n 10',
      stdout: 'line 1\nline 2\n',
    ...
    expect(text).toContain('Host evidence action: rbclaw_service_logs');
    expect(text).toContain('Exit code: 1');
    expect(text).toContain('$ journalctl --user -u rbclaw --no-pager -n 10');
