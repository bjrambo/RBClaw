...
const ORIGINAL_CLAUDE_REVIEWER_READONLY =
  process.env.RBCLAW_CLAUDE_REVIEWER_READONLY;
...
afterEach(() => {
  if (ORIGINAL_CLAUDE_REVIEWER_READONLY == null) {
    delete process.env.RBCLAW_CLAUDE_REVIEWER_READONLY;
  } else {
    process.env.RBCLAW_CLAUDE_REVIEWER_READONLY =
...
describe('claude reviewer runtime guard', () => {
  ...
  it('detects claude host reviewer read-only mode from env + role', () => {
    ...
    process.env.RBCLAW_CLAUDE_REVIEWER_READONLY = '1';
  ...
  it('prepends a git wrapper to PATH for reviewer runtimes', () => {
    ...
    expect(env.PATH).toContain('rbclaw-reviewer-git-');
    expect(env.RBCLAW_REAL_GIT).toBeTruthy();
  ...
  it('prefers an executable HOME-scoped wrapper dir before tmp', () => {
    const homeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-reviewer-home-'),
    );
    try {
      ...
      expect(wrapperDir).toContain(
        path.join(homeDir, '.rbclaw-reviewer-runtime'),
      );
    } finally {
  ...
  it('allows mutating git commands in temp repos outside the protected workspace', () => {
    const protectedDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'rbclaw-protected-workspace-'),
    );
    const env = buildReviewerGitGuardEnv(
      {
        PATH: process.env.PATH,
        RBCLAW_WORK_DIR: protectedDir,
      },
      true,
    );
    const cwd = createTempRepo('rbclaw-reviewer-temp-repo-');
  ...
  it('blocks mutating git subcommands inside the protected reviewer work directory', () => {
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'rbclaw-reviewer-test-'));
    const env = buildReviewerGitGuardEnv(
      {
        PATH: process.env.PATH,
        RBCLAW_WORK_DIR: cwd,
    ...
    try {
      ...
      expect(stderr).toContain(
        'RBClaw reviewer runtime blocks mutating git subcommands: commit',
      );
    }
  });

  it('overrides problematic git config paths so read-only git queries still work', () => {
    const cwd = createTempRepo('rbclaw-reviewer-readonly-git-');
    const env = buildReviewerGitGuardEnv(
      {
        PATH: process.env.PATH,
        HOME: cwd,
...
describe('claude reviewer work directory origin connectivity', () => {
  it('accepts a mounted local origin path that resolves as a git repo', () => {
    const originDir = createTempRepo('rbclaw-reviewer-origin-');
    const cwd = createTempRepo('rbclaw-reviewer-workspace-');
  ...
  it.each([
    'https://fixture.invalid/example/repository.git',
    'git@fixture.invalid:example/repository.git',
  ])(
    'accepts remote origin %s without requiring a mounted local canonical path',
    (originUrl) => {
      const cwd = createTempRepo('rbclaw-reviewer-remote-origin-');
  ...
  it('fails fast when the local origin path is not mounted as a git repo', () => {
    ...
    const missingOriginDir = path.join(
      os.tmpdir(),
      `rbclaw-reviewer-missing-origin-${Date.now()}`,
    );
    ...
    expect(() => assertReadonlyWorkspaceRepoConnectivity(env, true)).toThrow(
      `RBClaw readonly runtime cannot access local git origin path: ${missingOriginDir}`,
    );
  });
});
