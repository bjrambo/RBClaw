 ...
 *   - Keep RBClaw self-contained (no dependency on a host `claude` install).
 *
 * Override: set `RBCLAW_CLAUDE_CLI_PATH` to a custom absolute binary path.
 */
...
const ENV_OVERRIDE = 'RBCLAW_CLAUDE_CLI_PATH';
const GLM_CODE_ENV_OVERRIDE = 'RBCLAW_GLM_CODE_CLI_PATH';
 ...
 *
 * Resolution order:
 *   1. `RBCLAW_CLAUDE_CLI_PATH` env var (if set and file exists).
