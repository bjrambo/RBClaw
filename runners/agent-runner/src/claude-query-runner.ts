...
import { RBCLAW_ENV, IPC_POLL_MS } from 'rbclaw-runners-shared';
...
class ClaudeQueryRunner {
  ...
  constructor(private readonly args: RunClaudeQueryArgs) {
    ...
    if (!args.paths.workDir) {
      throw new Error('Channel workDir is required for agent execution.');
    }
    this.effectiveCwd = args.paths.workDir;
  }
  ...
  private buildQueryOptions() {
    ...
    const readonlyProtectedPaths = [this.effectiveCwd];
    ...
    return {
      ...
      mcpServers: {
        rbclaw: buildRbclawMcpServerConfig(this.args.mcpServerPath, {
  ...
  private resolveAgentType(): ClaudeCompatibleAgentType {
    return process.env[RBCLAW_ENV.agentType] === 'glm-code'
      ? 'glm-code'
      : 'claude-code';
  }
