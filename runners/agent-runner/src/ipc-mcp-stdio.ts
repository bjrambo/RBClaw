...
import { registerRoomAssignmentTool } from './ipc-room-assignment-tool.js';
import {
...
const REPO_ROOT = process.env[RBCLAW_ENV.workDir];
if (!REPO_ROOT) throw new Error('Channel workDir is required.');
// Context from environment variables (set by the agent runner)
const chatJid = process.env[RBCLAW_ENV.chatJid]!;
const groupFolder = process.env[RBCLAW_ENV.groupFolder]!;
const isMain = process.env[RBCLAW_ENV.isMain] === '1';
const agentType = process.env[RBCLAW_ENV.agentType] || 'claude-code';
...
server.tool(
  ...
  async (args) => {
    const data = buildSendMessageIpcPayload({
      ...
      senderRole: process.env[RBCLAW_ENV.roomRole] || undefined,
      runId: process.env[RBCLAW_ENV.runId] || undefined,
      groupFolder,
    });
...
registerHostEvidenceTools({
  ...
  groupFolder,
  roomRole: resolveCurrentRoomRole(roomRole),
  pairedTaskId: process.env[RBCLAW_ENV.pairedTaskId] || undefined,
  writeIpcFile,
});
