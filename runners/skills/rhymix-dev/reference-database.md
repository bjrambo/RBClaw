...
## Direct DB Insertion (MySQL CLI / Test Data)

When inserting data directly via MySQL CLI (e.g., for test data), you MUST specify `--default-character-set=utf8mb4` to avoid double-encoding of Korean/CJK characters. Rhymix uses `utf8mb4` charset — without this flag, the MySQL client defaults to `latin1`, causing UTF-8 bytes to be re-encoded.

**WRONG — causes mojibake (이중 인코딩):**
```bash
mysql -u user -p dbname -e "INSERT INTO rx_table (...) VALUES ('한글 데이터', ...);"
```

**CORRECT:**
```bash
mysql -u user -p --default-character-set=utf8mb4 dbname -e "INSERT INTO rx_table (...) VALUES ('한글 데이터', ...);"
```

**PREFERRED — use Rhymix framework queries (handles encoding automatically):**
```php
$args = new \stdClass;
$args->item_srl = getNextSequence();
$args->title = '테스트 데이터';
$output = executeQuery('module_name.insertItem', $args);
```

### Key rules for direct DB operations:
- Always use `--default-character-set=utf8mb4` with mysql CLI
- Table prefix is defined in `files/config/config.php` (e.g., `rx_`)
- `regdate` and date columns use `char(14)` format: `DATE_FORMAT(NOW(), '%Y%m%d%H%i%s')`
- Serial numbers (`*_srl`) should use `getNextSequence()` in PHP; for direct SQL use a large unique number
- Prefer Rhymix `executeQuery()` over raw SQL whenever possible — it handles charset, escaping, and table prefix automatically
