...
describe('shared agent output normalization', () => {
  ...
  it('parses silent rbclaw envelopes', () => {
    expect(
      normalizeRbclawStructuredOutput(
        JSON.stringify({
          rbclaw: { visibility: 'silent', verdict: 'silent' },
        }),
      ),
    ).toEqual({
      result: null,
  ...
  it('parses public rbclaw attachments', () => {
    expect(
      normalizeRbclawStructuredOutput(
        JSON.stringify({
          rbclaw: {
  ...
  it('parses fenced public rbclaw attachments', () => {
    expect(
      normalizeRbclawStructuredOutput(`\`\`\`json
{
  "rbclaw": {
    "visibility": "public",
    "text": "검 아이콘을 생성했습니다.",
...
\`\`\``),
  ...
  it('parses fenced rbclaw envelopes with leading invisible control characters', () => {
    expect(
      normalizeRbclawStructuredOutput(`\u2063\u2063\u2063\`\`\`json
{
  "rbclaw": {
    ...
    "attachments": [
      {
        "path": "/tmp/rbclaw-discord-image-final.png",
        "name": "final.png",
        "mime": "image/png"
      }
    ]
...
\`\`\``),
  ...
  it('parses status-prefixed fenced public rbclaw attachments', () => {
    expect(
      normalizeRbclawStructuredOutput(`TASK_DONE
...
\`\`\`json
{
  "rbclaw": {
    ...
    "attachments": [
      {
        "path": "/tmp/rbclaw-discord-image-status.png",
        "name": "status.png",
        "mime": "image/png"
      }
    ]
...
describe('shared agent protocol fallback behavior', () => {
  it('parses in_progress public envelopes instead of leaking raw structured JSON', () => {
    const raw = JSON.stringify({
      rbclaw: {
        ...
        attachments: [
          {
            path: '/tmp/rbclaw-discord-image-draft.png',
            name: 'draft.png',
            mime: 'image/png',
          },
        ],
    ...
    const normalized = normalizeRbclawStructuredOutput(raw);
  ...
  it('parses unlabeled fenced rbclaw envelopes', () => {
    expect(
      normalizeRbclawStructuredOutput(`\`\`\`
{"rbclaw":{"visibility":"public","text":"스크린샷입니다.","verdict":"done"}}
\`\`\``),
    ).toEqual({
      result: '스크린샷입니다.',
      output: {
  ...
  it('does not parse mixed prose and fenced rbclaw JSON as structured output', () => {
    const raw = `설명입니다.

\`\`\`json
{"rbclaw":{"visibility":"public","text":"첨부입니다.","verdict":"done"}}
\`\`\``;

    expect(normalizeRbclawStructuredOutput(raw)).toEqual({
      result: raw,
      output: {
        visibility: 'public',
        text: raw,
  ...
  it('falls back when fenced JSON has no rbclaw envelope', () => {
    const raw = `\`\`\`json
{"text":"plain json"}
\`\`\``;
  ...
  it('falls back when multiple fenced JSON blocks are present', () => {
    const raw = `\`\`\`json
{"rbclaw":{"visibility":"public","text":"첫 번째","verdict":"done"}}
\`\`\`

\`\`\`json
{"rbclaw":{"visibility":"public","text":"두 번째","verdict":"done"}}
\`\`\``;
