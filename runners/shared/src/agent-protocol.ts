export const OUTPUT_START_MARKER = '---RBCLAW_OUTPUT_START---';
export const OUTPUT_END_MARKER = '---RBCLAW_OUTPUT_END---';
...
export function normalizeAgentOutput(
  ...
  const normalized = normalizeRbclawStructuredOutput(result);
  if (
  ...
  if (explicitAttachments.length > 0) {
    return {
      ...normalized,
      attachmentSource: 'legacy-rbclaw-json',
    };
  }
