...
function createExecutableWrapperDir(baseEnv: NodeJS.ProcessEnv): string {
  const candidateRoots = [
    baseEnv.RBCLAW_REVIEWER_GIT_WRAPPER_ROOT,
    baseEnv.HOME ? path.join(baseEnv.HOME, '.rbclaw-reviewer-runtime') : null,
    path.join(os.tmpdir(), '.rbclaw-reviewer-runtime'),
  ...
  for (const candidateRoot of candidateRoots) {
    ...
    try {
      ...
      const wrapperDir = fs.mkdtempSync(
        path.join(candidateRoot, 'rbclaw-reviewer-git-'),
      );
...
export function buildReviewerGitGuardEnv(
  ...
  const protectedWorkDir = baseEnv.RBCLAW_WORK_DIR || '';
...
for blocked in "\${blocked_subcommands[@]}"; do
  if [[ "$is_protected_target" == "1" && "$subcmd" == "$blocked" ]]; then
    echo "RBClaw reviewer runtime blocks mutating git subcommands: $subcmd" >&2
    exit 1
  fi
done
exec "$real_git" "$@"
`;
  ...
  return {
    ...baseEnv,
    RBCLAW_REAL_GIT: realGitPath,
    RBCLAW_PROTECTED_WORK_DIR: protectedWorkDir,
...
export function assertReadonlyWorkspaceRepoConnectivity(
  ...
  if (!path.isAbsolute(originUrl) || !fs.existsSync(originUrl)) {
    throw new Error(
      `RBClaw readonly runtime cannot access local git origin path: ${originUrl || '(missing)'}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `RBClaw readonly runtime origin path is not mounted as a git repository: ${originUrl}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `RBClaw readonly runtime cannot resolve local git origin from ${protectedWorkDir}. Check canonical repo mount for ${originUrl}.`,
    );
  }
}
