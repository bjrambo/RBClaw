...
Build a personal Android companion for RBClaw that reuses the existing server and dashboard APIs, then extend it to Meta Ray-Ban Display through the Meta Wearables Device Access Toolkit (DAT) when the device developer preview is available to the account.

## Current Platform Facts

- RBClaw is a Bun/Node host service. Android should be a thin client, not a port of the runtime.
...
1. Keep RBClaw running on the existing server.
...
Do not expose RBClaw directly to the internet without a private transport.
...
- `RBClawApi`: HTTP client with bearer token.
...
The first build does not need DAT to validate RBClaw connectivity. DAT comes after the phone client can read rooms and send one message.
