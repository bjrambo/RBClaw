...
import { readDashboardToken, writeDashboardToken } from './api';
import { isLocale, localeTags, matchLocale, type Locale } from './i18n';
import { type DashboardView } from './DashboardNav';

const LOCALE_STORAGE_KEY = 'rbclaw.dashboard.locale.v2';
...
function persistNickname(trimmed: string): void {
  if (typeof window === 'undefined') return;
  if (trimmed) window.localStorage.setItem('rbclaw-nickname', trimmed);
  else window.localStorage.removeItem('rbclaw-nickname');
}
...
export function useDashboardChrome(): {
  ...
  nickname: string;
  online: boolean;
  setDashboardToken: (next: string) => void;
  ...
  const [nickname, setNicknameState] = useState<string>(() => {
    if (typeof window === 'undefined') return '';
    return window.localStorage.getItem('rbclaw-nickname') ?? '';
  });
  const [dashboardToken, setDashboardTokenState] = useState(readDashboardToken);
  ...
  function setDashboardToken(next: string) {
    setDashboardTokenState(next);
    writeDashboardToken(next);
  }
