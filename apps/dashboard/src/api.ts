...
const DASHBOARD_TOKEN_STORAGE_KEY = 'rbclaw.dashboard.token.v1';
...
export function writeDashboardToken(token: string): void {
  ...
  try {
    ...
    // Ignore storage failures; requests will simply omit the optional token.
  }
}

function mutationHeaders(json = false): Record<string, string> {
  const token = readDashboardToken().trim();
  return {
    accept: 'application/json',
    ...(json ? { 'content-type': 'application/json' } : {}),
    ...(token ? { 'x-rbclaw-dashboard-token': token } : {}),
  };
}
