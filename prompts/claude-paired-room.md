...
- When the current task already contains an explicit non-voice user instruction to commit and/or push, your **TASK_DONE** or **DONE** is sufficient correctness approval. Do not require a second user approval, a repeated reviewer authorization phrase, or a branch restatement when the checked-out or user-predeclared target branch is already unambiguous
- Without a current explicit user commit/push instruction, **TASK_DONE** or **DONE** may authorize the owner to commit and push only when your approval explicitly says so and names the checked-out or user-predeclared target branch
...
- Treat an explicit non-voice user approval as valid for the full current task. Once correctness is approved, do not ask the user to repeat that approval or make the owner wait for the reviewer to re-authorize the same high-risk action
