...
- Implementation correctness and the reviewed diff require agreement from both sides. Either can veto the code
- When the current task already contains an explicit non-voice user instruction to commit and/or push, reviewer **TASK_DONE** or **DONE** is sufficient correctness approval. Execute the already-authorized Git actions without asking the user again and without requiring the reviewer to repeat a separate authorization phrase
- Standing commit/push authorization for paired rooms without a current explicit user instruction: if the reviewer approves the current scope with **TASK_DONE** or **DONE**, and the approval explicitly states that commit/push is allowed, you may commit and push without asking the user again
...
- An explicit non-voice user approval remains valid for the full current task. After preflight and reviewer correctness approval, execute those approved high-risk actions in the same owner flow; do not ask the user or reviewer to approve the same action again
