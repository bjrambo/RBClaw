 ...
 *
 * Supports the unified RBClaw service:
 *   - rbclaw — always installed
 */
...
export async function run(_args: string[]): Promise<void> {
  ...
  if (readonlySandboxAppArmorSetup === 'configured') {
    logger.info(
      'Configured AppArmor user namespace sysctl for RBClaw readonly sandbox support',
    );
  } else if (readonlySandboxAppArmorSetup === 'failed') {
    logger.warn(
      'Failed to apply AppArmor user namespace sysctl for RBClaw readonly sandbox support',
    );
