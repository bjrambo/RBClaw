...
export const STACK_RESTART_UNIT_NAME = 'rbclaw-stack-restart.service';
...
const MANAGED_SERVICE_CALLER_FALLBACK_MESSAGE =
  'Stack restart unit is not installed yet. Run `bun run setup -- --step service` from an external shell before retrying from a managed RBClaw service.';
...
function restartStackServicesDirect(
  ...
  if (services.length === 0) {
    throw new Error('No RBClaw services are configured in this project');
  }
