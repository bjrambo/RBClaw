...
describe('verify state summary building', () => {
  it('builds a successful verification summary when all gates pass', () => {
    const services: ServiceCheck[] = [{ name: 'rbclaw', status: 'running' }];
    ...
    expect(
      ...
      servicesSummary: { rbclaw: 'running' },
    });
  });

  it('fails verification when any required gate is missing', () => {
    const services: ServiceCheck[] = [{ name: 'rbclaw', status: 'stopped' }];
    ...
    expect(
      ...
      status: 'failed',
      configuredChannels: [],
      servicesSummary: { rbclaw: 'stopped' },
    });
  });
