...
const baseServiceDef: ServiceDef = {
  kind: 'primary',
  description: 'RBClaw Personal Assistant',
...
describe('plist generation', () => {
  it('contains the correct label', () => {
    ...
    expect(plist).toContain('<string>com.rbclaw</string>');
  });
  ...
  it('points to dist/index.js', () => {
    ...
    expect(plist).toContain('/home/user/rbclaw/dist/index.js');
  });
  ...
  it('sets log paths', () => {
    ...
    expect(plist).toContain('rbclaw.log');
    expect(plist).toContain('rbclaw.error.log');
  });
});
...
describe('systemd unit generation', () => {
  ...
  it('sets correct ExecStart', () => {
    ...
    expect(unit).toContain('ExecStart=/usr/bin/bun /srv/rbclaw/dist/index.js');
  });
  ...
  it('preserves EnvironmentFile and extraEnv in the actual builder', () => {
    const unit = buildSystemdUnit(
      {
        ...baseServiceDef,
        kind: 'primary',
        environmentFile: '/srv/rbclaw/.env.extra',
    ...
    expect(unit).toContain('EnvironmentFile=/srv/rbclaw/.env.extra');
...
describe('WSL nohup fallback', () => {
  it('generates a valid wrapper script', () => {
    const projectRoot = '/home/user/rbclaw';
    const nodePath = '/usr/bin/node';
    const pidFile = path.join(projectRoot, 'rbclaw.pid');

    // Simulate what service.ts generates
    const wrapper = `#!/bin/bash
set -euo pipefail
cd ${JSON.stringify(projectRoot)}
nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot)}/dist/index.js >> ${JSON.stringify(projectRoot)}/logs/rbclaw.log 2>> ${JSON.stringify(projectRoot)}/logs/rbclaw.error.log &
echo $! > ${JSON.stringify(pidFile)}`;

    expect(wrapper).toContain('#!/bin/bash');
    expect(wrapper).toContain('nohup');
    expect(wrapper).toContain(nodePath);
    expect(wrapper).toContain('rbclaw.pid');
  });
});
...
describe('service definitions', () => {
  ...
  it('returns only the unified service definition', () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'rbclaw-stack-'));
    tempRoots.push(tempRoot);

    const defs = getServiceDefs(tempRoot);

    expect(defs.map((def) => def.name)).toEqual(['rbclaw']);
  ...
  it('generates a oneshot stack restart unit', () => {
    ...
    expect(unit).toContain('Description=RBClaw Stack Restart Orchestrator');
    expect(unit).toContain('Type=oneshot');
    expect(unit).toContain(
      'ExecStart=/usr/bin/bun /srv/rbclaw/setup/restart-stack.ts --direct',
    );
  });
});
